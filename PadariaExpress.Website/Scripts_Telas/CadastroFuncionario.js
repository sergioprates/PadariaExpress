...
function gravar(event) {
    ...
    if ($("#cadastroForm").valid()) {
        ...
        var url = 'API/funcionario';

        if (IsEmpty(model.UsuarioId) == false) {
            url += '/atualizar'
        }

        chamadaAPI(url, 'POST', model, function (response) {
            var usuario = response.d;
            window.location = 'Funcionarios.html?PadariaId=' + querystring('PadariaId');
        }, function (erro) {
            ocultaAguarde();
            if (IsEmpty(erro.Message) == false) {
...
function carregaUsuario(usuario) {
    if (IsEmpty(querystring('UsuarioId')) == false) {
        var model = {};
        chamadaAPI('api/funcionario/buscar/' + querystring('UsuarioId'), 'GET', model, function (response) {
            var usuario = response.d;
...
function mapearUsuario() {
    ...
    usuario.UsuarioId = querystring('UsuarioId');
    usuario.PadariaId = querystring('PadariaId');
    return usuario;
}
